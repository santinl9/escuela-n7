...
import type { AsistenciaInstitucional } from '../types/AsistenciaInstitucional';
...
// Detalle de un curso desde "Mis Cursos" (Seguimiento y Evaluación): listado de sus
// matriculados, de solo lectura, con acceso a pasar lista. No depende de Estructura
// Institucional para no mezclar ambos flujos.
function DetalleMiCurso() {
  ...
  const { data: asistenciasData, loading: loadingAsistencias, error: errorAsistencias } = useFetch<AsistenciaInstitucional[]>('/data/asistenciasInstitucionales.json');
  ...
  const hoy = hoyISO();
  const yaPasoListaHoy = matriculados.length > 0 && matriculados.some(({ estudiante }) =>
    (asistenciasData ?? []).some(a => a.estudianteDni === estudiante.dni && a.fecha === hoy)
  );
    ...
    <div className="flex flex-col gap-8">
      ...
      <div className="flex flex-col gap-3">
        ...
        <div className="flex items-start justify-between gap-4 flex-wrap">
          ...
          <button
            onClick={() => navigate(`/seguimiento-evaluacion/mis-cursos/${curso.id}/pasar-lista`)}
            title={yaPasoListaHoy ? 'Ya se pasó lista hoy en este curso: podés editarla' : undefined}
          ...
          >
            <span className="material-symbols-outlined text-[1.25rem]">{yaPasoListaHoy ? 'edit_calendar' : 'checklist'}</span>
            {yaPasoListaHoy ? 'Editar Lista de Hoy' : 'Pasar Lista'}
          </button>
        </div>
      </div>
              ...
              <tbody className="divide-y divide-outline-variant">
                {matriculadosFiltrados.map(({ matricula, estudiante }) => (
                  <tr key={matricula.id} className="hover:bg-surface-container-high transition-colors duration-150">
                    ...
                    <td className="px-4 py-3">
                      <button
                        onClick={() => navigate(`/seguimiento-evaluacion/mis-cursos/${curso.id}/estudiantes/${estudiante.id}`)}
